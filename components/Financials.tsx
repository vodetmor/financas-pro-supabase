...
export const Financials: React.FC<FinancialsProps> = ({
  ...
  const handleSubSubmit = (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      nextPaymentDate: subForm.firstPaymentDate, // Reset next payment logic usually handled by backend/effect, here simplify
    ...
    if (editingSub) {
      // Preserve original nextPaymentDate on edit
            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
               <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-emerald-600 mt-2">+ R$ {totalIncome.toLocaleString('pt-BR')}</p>
                  </div>
            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
               <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-rose-600 mt-2">- R$ {totalExpense.toLocaleString('pt-BR')}</p>
                  </div>
                ...
                {sortedSubs.map((s) => {
                  ...
                  return (
                    <tr key={s.id} onClick={() => handleStartEdit(s)} className={`hover:bg-slate-50 transition-colors cursor-pointer ${!s.active ? 'opacity-50 grayscale' : ''}`}>
                      ...
                      <td className="px-6 py-4 font-bold text-slate-800 whitespace-nowrap">R$ {s.amount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {s.autoPay 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
           <div className="bg-white rounded-xl shadow-lg w-full max-w-md p-6">
              <div className="flex justify-between items-center mb-4">
                 ...
                 <button onClick={() => setIsTxModalOpen(false)} className="text-slate-400 hover:text-slate-600"><X size={20}/></button>
              </div>
