...
export const DailyCheck: React.FC<DailyCheckProps> = ({ data, onUpdateOffer, visualizationCurrency }) => {
                ...
                <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-lg border border-slate-200 shadow-sm">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="w-24 h-2 bg-slate-100 rounded-full overflow-hidden">
                            <div
                                className={`h-full rounded-full transition-all duration-500 ${complianceRate === 100 ? 'bg-emerald-500' :
                                    complianceRate > 50 ? 'bg-amber-500' : 'bg-rose-500'
                                    }`}
                                style={{ width: `${complianceRate}%` }}
                            />
                        </div>
                        <span className={`font-bold ${complianceRate === 100 ? 'text-emerald-600' :
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 relative">
                        ...
                        <form onSubmit={handleResolve} className="space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Faturamento ({selectedTask?.offer.currency || 'BRL'})</label>
                                <input
                            ...
                            <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Investimento Ads ({selectedTask?.offer.currency || 'BRL'})</label>
                                <input
