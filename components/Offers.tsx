...
export const Offers: React.FC<OffersProps> = ({ data, onAddOffer, onUpdateOffer, onDeleteOffer, visualizationCurrency, setVisualizationCurrency }) => {
  ...
  const processGlobalChartData = () => {
    ...
    return Array.from(dataMap.entries()).sort().map(([key, val]) => {
      ...
      return {
        ...
        ads: convertFromBRL(val.ads),
        profit: convertFromBRL(val.profit),
        commission: convertFromBRL(val.commission)
      };
    });
  };
  ...
  const globalChartData = useMemo(() => processGlobalChartData(), [data.offers, globalDateFilter, globalCustomDates, globalViewGrouping, visualizationCurrency]);
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-6">
              ...
              <div className="flex bg-slate-100 border border-slate-200 rounded-lg p-1 relative">
                {['TODAY', 'WEEK', 'MONTH', 'ALL'].map((f) => (
                  <button
                    ...
                    className={`px-3 py-1 text-xs font-semibold rounded transition-colors ${globalDateFilter === f ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-200'
                      }`}
                  >
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded transition-colors flex items-center justify-center ${showGlobalCustomPicker ? 'bg-slate-900 text-white' : 'text-slate-500 hover:bg-slate-200'
                    }`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
              ...
              <p className="text-xl font-bold text-blue-600 mt-1">{formatCurrency(globalFilteredTotals.revenue, visualizationCurrency)}</p>
            </div>
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
              <p className="text-xs font-bold text-slate-400 uppercase">Despesa Total (Ads)</p>
              <p className="text-xl font-bold text-rose-600 mt-1">{formatCurrency(globalFilteredTotals.ads, visualizationCurrency)}</p>
            </div>
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
              <p className="text-xs font-bold text-slate-400 uppercase">Lucro Líquido (Ofertas)</p>
              <p className="text-xl font-bold text-emerald-600 mt-1">{formatCurrency(globalFilteredTotals.profit, visualizationCurrency)}</p>
            </div>
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
              <p className="text-xs font-bold text-slate-400 uppercase">Comissão Equipe</p>
              <p className="text-xl font-bold text-indigo-600 mt-1">{formatCurrency(globalFilteredTotals.commission, visualizationCurrency)}</p>
            </div>
          </div>
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4 border-t border-slate-100 pt-4 mb-4">
                <div>
                  ...
                  <span className="font-bold text-blue-600 text-lg">{formatCurrency(convertFromBRL(metrics.revenue), visualizationCurrency)}</span>
                </div>
                <div>
                  ...
                  <span className={`font-bold text-lg ${metrics.profit >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                    {formatCurrency(convertFromBRL(metrics.profit), visualizationCurrency)}
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col md:flex-row justify-between items-center bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
                ...
                <div className="flex items-center gap-2 relative">
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded transition-colors flex items-center justify-center ${showCustomPicker ? 'bg-slate-900 text-white' : 'text-slate-500 hover:bg-slate-50'
                      }`}
                    title="Período Personalizado"
                  >
                    <Calendar size={14} />
...
const CreateOfferModal = ({ onClose, onSave }: { onClose: () => void, onSave: (o: Omit<Offer, 'id'>) => void }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
      ...
      originalAmount: 0, // Offers track accumulated value, but we store creation currency
      description: '',
      ...
      <div className="bg-white rounded-xl shadow-lg w-full max-w-md p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Modelo de Comissão</label>
              <select
              ...
              >
                <option value={PayoutModel.REVENUE}>% sobre FATURAMENTO</option>
                <option value={PayoutModel.PROFIT}>% sobre LUCRO</option>
              </select>
            </div>
            <div>
