                     ...
                     <div className="flex bg-slate-100 border border-slate-200 rounded-lg p-1 relative">
                        {['TODAY', 'WEEK', 'MONTH', 'ALL'].map((f) => (
                            <button
                                ...
                                dateFilter === f ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-200'
                                }`}
                            >
                        ...
                        <button
                            ...
                            className={`px-3 py-1 rounded transition-colors flex items-center justify-center ${
                                showCustomPicker ? 'bg-slate-900 text-white' : 'text-slate-500 hover:bg-slate-200'
                            }`}
        ...
        <div className="p-6 border-b border-slate-100 flex justify-between items-start bg-slate-50">
          ...
          <div className="flex items-center gap-3">
            <select 
              ...
              className={`text-sm font-semibold py-1.5 px-3 rounded-lg border outline-none cursor-pointer ${
                localService.status === ServiceStatus.COMPLETED ? 'bg-emerald-100 text-emerald-800 border-emerald-200' :
                  ...
                  {localService.steps.map(step => (
                    <div key={step.id} className="flex items-center group">
                      <button 
                        ...
                        className={`flex-shrink-0 w-6 h-6 rounded-full border flex items-center justify-center transition-all mr-3 ${
                          step.isCompleted 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-xl shadow-lg w-full max-w-md p-6">
        <h3 className="text-lg font-bold text-slate-900 mb-4">Novo Projeto / Serviço</h3>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Nome do Cliente</label>
            <input required autoFocus className="w-full bg-white text-slate-900 border border-slate-300 rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 outline-none" value={form.clientName} onChange={e => setForm({...form, clientName: e.target.value})} placeholder="Ex: TechCorp" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Título do Serviço</label>
            <input required className="w-full bg-white text-slate-900 border border-slate-300 rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 outline-none" value={form.title} onChange={e => setForm({...form, title: e.target.value})} placeholder="Ex: Consultoria de Vendas" />
          </div>
          <div className="grid grid-cols-2 gap-4">
             <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Valor Total (R$)</label>
              <input required type="number" className="w-full bg-white text-slate-900 border border-slate-300 rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 outline-none" value={form.value} onChange={e => setForm({...form, value: Number(e.target.value)})} />
            </div>
            <div>
